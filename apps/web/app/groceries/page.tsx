...
interface Recipe { id: string; key_ingredients: string[] }
...
export default function GroceriesPage() {
  ...
  useEffect(() => {
    ...
    if (!rawPlan) { setError("Build a weekly plan before generating groceries."); return; }
    ...
    if (savedItems) { try { setItems(JSON.parse(savedItems) as GroceryItem[]); return; } catch { window.localStorage.removeItem(groceryStorageKey); } }
    const plan = JSON.parse(rawPlan) as MealPlan;
    const uniqueIngredients = [...new Set(plan.meals.flatMap((meal) => meal.key_ingredients.map((ingredient) => ingredient.trim()).filter(Boolean)))];
    if (uniqueIngredients.length === 0) { setError("Your selected recipes do not have ingredients to sort yet."); return; }
    ...
    }).catch((requestError: unknown) => setError(requestError instanceof Error ? requestError.message : "Could not categorize your groceries."));
  }, []);
  ...
  return <main><Link className="brand" href="/">NextBite</Link><section className="recommendation-header"><p className="eyebrow">Shopping list</p><h1>Groceries, sorted your way.</h1><p className="lede">OpenAI sorts ingredients into your preferred stores and categories. Drag an item to make any final adjustment.</p><button onClick={() => void copyIngredients()} type="button">Copy ingredients</button><p aria-live="polite" className="copy-status">{copyStatus}</p></section>{error ? <section className="error-card"><p>{error}</p><Link className="button" href="/plan">Build a plan</Link></section> : <div className="grocery-board">{stores.map((store) => <section className="store-column" key={store} onDragOver={(event) => event.preventDefault()} onDrop={(event) => moveItem(event, store)}><h2>{store}</h2>{["Produce", "Meat & protein", "Dairy", "Bakery", "Frozen", "Pantry", "Other"].map((category) => { const categoryItems = items.filter((item) => item.store === store && item.category === category); return categoryItems.length > 0 ? <div className="grocery-category" key={category}><h3>{category}</h3>{categoryItems.map((item) => <label className={`grocery-item ${item.checked ? "grocery-item-checked" : ""}`} draggable key={item.id} onDragStart={(event) => event.dataTransfer.setData("text/plain", item.id)}><input checked={item.checked} onChange={() => toggleItem(item.id)} type="checkbox" />{item.name}</label>)}</div> : null; })}</section>)}</div>}</main>;
}
