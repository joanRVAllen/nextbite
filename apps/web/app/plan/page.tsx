...
export default function PlanPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`${apiUrl}/api/meal-plans/generate`, {
        ...
        body: JSON.stringify({ favorite_meals: profile.favoriteMeals, meals_per_week: Number(profile.mealsPerWeek), priorities: profile.planningPriorities ?? ["Fastest cooking", "Maximum variety", "Grocery waste"], favorite_recipes: favorites.map(({ id, title, cuisine, prep_minutes, key_ingredients }) => ({ id, title, cuisine, prep_minutes, key_ingredients, source: "favorite" })), new_recommendations: approvedRecipes.map(({ id, title, cuisine, prep_minutes, key_ingredients }) => ({ id, title, cuisine, prep_minutes, key_ingredients, source: "new" })) })
      })
      .then(async (response) => {
